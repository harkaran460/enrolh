...
<div class="page-content">
    ...
    <div class="container-fluid pt-5">
        <?php
        if(Session::get('program_updated') !== null){
            echo '<div class="alert alert-success" role="alert">';
            ?>
            {{Session::get('program_updated')}}
            <?php
        ...
        <div class="row">
                            ...
                            <table class="table">
                                <thead>
                                    <tr style="border-bottom: 2px solid #0000001f;">
                                        ...
                                        <th class="" scope="col">Program Name</th>
                                        <th class="" scope="col">Program College Name</th>
                                        <th class="" scope="col">Intake Date</th> 
                                        <th class="" scope="col">Intake Type</th>  
                                        <th class="" scope="col">View More</th>
                                    </tr> 
                                </thead>
                                <tbody>

                                    @if(!empty($list_of_programs))
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                                    <li><a class="dropdown-item"  href="admin-view-program/{{$program->id}}"><i class="bi bi-pencil"></i> View Application</a></li>
                                                    <li><a class="dropdown-item"  href="admin-edit-program/{{$program->id}}"><i class="bi bi-pencil"></i> Edit Application</a></li>
                                                </ul>
                                            </div>
                                        </td>
                                        <td class="">{{$program->college_name}}</td>
                                        <td class="">{{$program->programs_name}}</td>
                                        <td class="">{{$program->program_college_name}}</td>
                                        <td class="">{{$program->earliest_intake_date}}</td> 
                                        <td class="">{{$program->earliest_intake_type}}</td>  
                                        <td class="">
                                            <span class="application_view"><a href="admin-view-college/">View</a></span> 
                                        </td>
                                    </tr>
                                    @endforeach
                                    @endif
            ...
            <ul class="pagination">{{$list_of_programs->links('pagination::bootstrap-4')}}</ul>
        </div>   
            </div> 
